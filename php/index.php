...
if (isset($_POST["weather"])) {
  $submit = $_POST["weather"];
  if ( $submit == 'on' ) {
      $output = shell_exec('sudo systemctl start weather');
      sleep(5);
      echo $output;
  }
  if ( $submit == 'off' ) {
      $output = shell_exec('sudo systemctl stop weather');
      echo $output;
  }
}
...
echo "&nbsp;";
echo "</td><td>";
echo "<font size=+1>JS8Call</font> (Pi3/4)</td>";
echo '<td align="right" nowrap>';
...
echo "<tr>";
$output = shell_exec('systemctl is-active weather');
#$output = str_replace("failed", "inactive", $output);
$output = chop($output);
  if ($output == "active")
  {
...
echo "&nbsp;";
echo "</td><td>";
echo "<font size=+1>Weather</font></td>";
echo '<td align="right" nowrap>';
echo '<input type="submit" name="weather" value="on"> ';
echo '<input type="submit" name="weather" value="off">';
echo "</font>";
echo "</td></tr>";
...
$output = shell_exec('sudo systemctl reset-failed fldigi');
$output = shell_exec('sudo systemctl reset-failed sstv');
$output = shell_exec('sudo systemctl reset-failed wsjtx');
$output = shell_exec('sudo systemctl reset-failed ardop');
$output = shell_exec('sudo systemctl reset-failed tnc300b');
$output = shell_exec('sudo systemctl reset-failed digipeater');
$output = shell_exec('sudo systemctl reset-failed tnc');
$output = shell_exec('sudo systemctl reset-failed node');
$output = shell_exec('sudo systemctl reset-failed winlinkrms');
$output = shell_exec('sudo systemctl reset-failed pat');
$output = shell_exec('sudo systemctl reset-failed js8call');
$output = shell_exec('sudo systemctl reset-failed gpsgrid');
$output = shell_exec('sudo systemctl reset-failed weather');
...
<tr>
  <td>
    <a href="/ft8" target="ft8" title="Dispaly FT8 screen"><strong>FT8</strong>
  </td>
  <td>
...
<?php
  if (!file_exists("/var/cache/digipi/localized.txt")) {
    echo '<tr><td colspan=3><a href="/setup.php" title="REQUIRED!  Enter your callsign and other local information" " target="setup"><font color="green"><strong>Initialize</strong><>
  }
?>
...
if (isset($_POST["reboot"])) {
  ...
  if ( $submit == 'Reboot' ) {
      ...
      #$output = shell_exec("echo Reboot stub");
      echo $output;
  }
}
...
if (isset($_POST["shutdown"])) {
  ...
  if ( $submit == 'Shutdown' ) {
      ...
      #$output = shell_exec("echo Shutdown stub");
      echo $output;
  }
}
